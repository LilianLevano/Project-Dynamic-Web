...
const fetchData = async (withAllLanguage = false) => {
  try {
      ...
      !res.ok ? (() => { throw new Error("Could not fetch");})(): null;
...
// na het geven van id gaat de naamlocations sorteren alfabetisch
...
const updateMainList = (arrayResults) => {
  ...
  for (let locatie of arrayResults) {
    ...
    const divInformatieText = makeCard(locatie, lat, lon)
...
const makeSearchCover = (callback) => {
  ...
  const path = document.createElementNS(ns, "path");
  path.setAttribute("d","m251.33-204.67-46.66-46.66L433.33-480 204.67-708.67l46.66-46.66L480-526.67l228.67-228.66 46.66 46.66L526.67-480l228.66 228.67-46.66 46.66L480-433.33 251.33-204.67Z",);
...
const updateLijstInSearchCover = (arrayLocations) => {
  ...
  if (arrayLocations.length == 0) {
    ...
    for (let locatie of arrayLocations) {
      ...
      const divInformatieText = makeCard(locatie, lat, lat)
...
const makeCard = (locatie, lat, lon) =>{
...
for (let button of buttonTalen) {
  button.addEventListener("click", async () => {
    ...
    // om na het wisselen van taal gaat het ook alfabetisch sorteren

    const nieuweTaal = localStorage.getItem("taal");
    allLocations.sort((a, b) =>
      (nieuweTaal === "nl" ? a.beschrijving : a.description).localeCompare(
        nieuweTaal === "nl" ? b.beschrijving : b.description,
      ),
    );
